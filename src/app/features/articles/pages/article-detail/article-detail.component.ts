...
export class ArticleDetailComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      const blocks = await this.articleService.getArticleBlocks(article.id);
  ...
  private setMetaTags(article: SoundArticle) {
    const title = this.getLocalizedArticleTitle() + ' — Ecnelis Fly';
    this.titleService.setTitle(title);
    this.meta.updateTag({ property: 'og:title', content: this.getLocalizedArticleTitle() });
    if (article.description) {
      this.meta.updateTag({
        property: 'og:description',
        content: this.getLocalizedArticleDescription(),
      });
    }
    if (this.coverUrl()) {
      this.meta.updateTag({ property: 'og:image', content: this.coverUrl()! });
    }
  ...
  async share() {
    ...
    const title = this.getLocalizedArticleTitle();
    ...
    if (navigator.share) {
      ...
      this.snackBar.open(
        this.translate.instant('articles.detail.shareSuccess'),
        '',
        { duration: 3000 },
      );
    }
